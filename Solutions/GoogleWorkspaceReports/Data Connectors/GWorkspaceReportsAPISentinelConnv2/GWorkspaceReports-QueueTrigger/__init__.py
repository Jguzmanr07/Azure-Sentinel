...
def get_result(activity,start_time, end_time, next_page_token=None):
    try:
        result_activities = []
        ...
        if next_page_token is not None and len(next_page_token) > 0:
            results = service.activities().list(userKey='all', applicationName=activity,
                                                maxResults=max_page_size, startTime=start_time, endTime=end_time, pageToken=next_page_token).execute()
        else:
            results = service.activities().list(userKey='all', applicationName=activity,
                                                maxResults=max_page_size, startTime=start_time, endTime=end_time).execute()
...
def main(queueItem: func.QueueMessage ):
    ...
    next_page_token = message_body.get('next_page_token')
    global creds
    ...
    try:
        result_obj, next_page_token = get_result(activity,start_time,end_time, next_page_token)
    ...
    try:
        if next_page_token is not None and len(next_page_token) > 0:
            logging.info("There are more events for {} activity with {} start time and {} end time".format(activity,start_time,end_time))
            queue_body["start_time"] = start_time
            queue_body["end_time"] = end_time
            queue_body["activity"] = activity
            queue_body["next_page_token"] = next_page_token
            #Write to queue with next_page_token
            ...
            if return_message is not None and return_message.id is not None:
                ...
                raise Exception("Message sent to queue with encoding with Base64. Message Id: {} Message Content: {} Queue Name: {}".format(return_message.id, queue_body, queue_name))
            else:
                raise Exception("Message not sent to queue. Message Content: {} Queue Name: {}".format(queue_body, queue_name))
    except Exception as err:
